###########################################################################################
###########################################################################################
## VALIDATE TUNED MODELS ACROSS YEARS
###########################################################################################
###########################################################################################
...
source("T:/PhD/Scripts/Functions/my_functions_work.R")
...
eval_pls_cross <- function(train, valid, data, sampling_train, 
                           sampling_val, preProc, maxcomp, bdir){
  ...
  for(k in names(data)){
    ...
    for (i in 1:length(train)) {
      ...
      if(sampling_train == "downsample"){
        ...
        dat_train <- dat_train0
        dat_train$SnsCnp <- as.factor(dat_train$SnsCnp)
        dat_train <- upSample(x = dat_train,
                              y = dat_train$SnsCnp,
                              yname = "SnsCnp")
        dat_train$SnsCnp <- as.numeric(as.character(dat_train$SnsCnp))
        dat_train <- dat_train[,-ncol(dat_train)]
      ...
      #create indices for data splitS
      train_index <- caret::createMultiFolds(y = dat$SnsCnp, k = 10, times = 5)
      
      # Define cross validation procedure
      ##  5 times repeated 10-fold cross validation is used to identify the model with the 
      ##  highest prediction accuracy on the hold-out samples. 
      ##  The best model is reduced by removing components until the increase in the RMSEP exceeds one standard error. 
      ...
      #######################################################################
        ...
        predobs_cv <- plyr::ldply(list(train_plsr),
        ...
        #calculate RMSE
        RMSE <- round(train_plsr$results[train_plsr$results$ncomp == train_plsr$bestTune[1, 1], "RMSE"], 2)
        ...
        pred <- caret::predict.train(train_plsr, newdata = dat_train0, 
      ...
      for(j in 1:length(valid[[i]])){
        ...
        #predict new data using trained model
        pred <- caret::predict.train(train_plsr, newdata = dat_val_sub, 
                                     type = "raw", na.action = na.omit)
        ...
        #predict new data using trained model
        pred <- caret::predict.train(train_plsr, newdata = dat_val0, 
                                     type = "raw", na.action = na.omit)
