...
dir <- "T:/PhD/DataAnalysis/FPWW012_FPWW018/ANOVA_yield/Results/"

#GY
load(paste(dir, "gen_predictors2016.rda", sep = ""))
BLUEs2016_gy <- pvGens
load(paste(dir, "gen_predictors2017.rda", sep = ""))
BLUEs2017_gy <- pvGens
load(paste(dir, "gen_predictors2018.rda", sep = ""))
BLUEs2018_gy <- pvGens

BLUEs2016_gy$Rep <- as.factor(1)
BLUEs2016_gy$Year <- as.factor(2016)
BLUEs2017_gy$Rep <- as.factor(2)
BLUEs2017_gy$Year <- as.factor(2017)
BLUEs2018_gy$Rep <- as.factor(3)
BLUEs2018_gy$Year <- as.factor(2018)

BLUEs_gy <- rbind(BLUEs2016_gy, BLUEs2017_gy, BLUEs2018_gy)

dir <- "T:/PhD/DataAnalysis/FPWW012_FPWW018/ANOVA_ind/Results/"

#DynPars
load(paste(dir, "gen_predictors2016.rda", sep = ""))
BLUEs2016_ind <- pvGens
load(paste(dir, "gen_predictors2017.rda", sep = ""))
BLUEs2017_ind <- pvGens
load(paste(dir, "gen_predictors2018.rda", sep = ""))
BLUEs2018_ind <- pvGens

BLUEs2016_ind$Rep <- as.factor(1)
BLUEs2016_ind$Year <- as.factor(2016)
BLUEs2017_ind$Rep <- as.factor(2)
BLUEs2017_ind$Year <- as.factor(2017)
BLUEs2018_ind$Rep <- as.factor(3)
BLUEs2018_ind$Year <- as.factor(2018)

BLUEs_ind <- rbind(BLUEs2016_ind, BLUEs2017_ind, BLUEs2018_ind)

dir <- "T:/PhD/DataAnalysis/FPWW012_FPWW018/ANOVA_scorings/Results/"
...
BLUEs2016_sc$Rep <- as.factor(1)
BLUEs2016_sc$Year <- as.factor(2016)
BLUEs2017_sc$Rep <- as.factor(2)
BLUEs2017_sc$Year <- as.factor(2017)
BLUEs2018_sc$Rep <- as.factor(3)
BLUEs2018_sc$Year <- as.factor(2018)

BLUEs_sc <- rbind(BLUEs2016_sc, BLUEs2017_sc, BLUEs2018_sc)

BLUEs <- list(BLUEs_gy, BLUEs_ind, BLUEs_sc) %>%
  Reduce(function(dtf1,dtf2) full_join(dtf1,dtf2,by=c("Gen", "Rep", "Year")), .)
...
# read data
  ...
  data <- xy %>% 
    ...
    tidyr::gather(Response, response, GY:GPC, factor_key = TRUE) %>%
    #exclude obs with missing data
    filter(complete.cases(.)) %>%
    #reorder columns
    select(Year, sendynpar, Response, value, response) %>%
    #create nested tibble
    tidyr::nest(value, response) %>%
    tibble::as.tibble() %>%
    ...
    arrange(Year, Response, desc(abs(estimate)))
  
  out <- data %>% select(1:3, estimate, pval) %>%
    rowwise() %>%
    mutate(sig = ifelse(pval < 0.001, "***", ifelse(pval < 0.01, "**", ifelse(pval < 0.05, "*", "")))) %>%
    transmute(Year, sendynpar, Response, Est = paste(round(estimate, 3), sig, sep = "")) 
